...
public class EnemigoMediano2 : MonoBehaviour
{
    ...
    void Update()
    {
        transform.Translate(velocidad * Time.deltaTime);// zaradi tova se razpryscvat ç
                                                        // Rotate the enemy sprite based on the direction of movement
        if (velocidad.y > 0) // Moving right
        {
            transform.localScale = new Vector3(2, -1, 1); // No flip needed
        }
        else if (velocidad.y < 0) // Moving left
        {
            transform.localScale = new Vector3(2, 1, 1); // Flip horizontally
        }
